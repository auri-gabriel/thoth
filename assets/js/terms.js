async function getRelatedTerms(term) {
	...
	*/
	...
	request = await fetch(
		"https://en.wikipedia.org/w/api.php?origin=*&action=parse&prop=sections&format=json&pageid=" +
			pageid,
		conf,
	);
	...
	request = await fetch(
		"https://en.wikipedia.org/w/api.php?origin=*&action=parse&prop=links&format=json&pageid=" +
			pageid +
			"&section=" +
			index,
		conf,
