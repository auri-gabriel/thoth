function add_general_quality_score() {
	...
	if (
		!validate_general_quality_score(
			start_interval,
			end_interval,
			general_score_desc,
		)
...
function validate_general_quality_score(
	start_interval,
	end_interval,
	general_score_desc,
	index,
			...
			table_qa.row
				.add([
					...
					'<table id="table_' +
						...
						"</table>",
					weight,
					'<select class="form-control" id="min_to_' +
						id +
						'" onchange="edit_min_score_qa(this)">' +
						'<option value=""></option>' +
						"</select>",
...
function validate_qa(id, qa, weight, index) {
	...
	let max = table_general_score.column(1).data().sort().reverse()[0];
...
function modal_score_quality(value) {
	...
	$("#modal_score_quality #edit_score").val(
		row.cells.item(1).innerHTML.replace("%", ""),
	);
	$("#modal_score_quality #edit_lbl_score").text(
		"Score: " + row.cells.item(1).innerHTML.replace("%", "") + "%",
	);
...
function validate_score_quality(
	score_rule,
	score,
	description,
	id_table,
...
function edit_score_quality() {
	...
	if (
		!validate_score_quality(score_rule, score, description, id_table, index)
	) {
		return false;
	}
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data["keywords"] != "") {
					...
					$("#paper_keywords_conf_qa").text(
						"This article does not have Keywords",
					);
				}
