				...
				$.ajax({
					...
					success: function () {
						...
						table_bib.parentNode.parentNode.cells.item(1).innerHTML =
							parseInt(p) + papers.entries.length;
						...
						if (papers.errors.length > 0) {
							Swal.fire({
								...
								html:
									...
									"</strong> was upload and <strong>" +
									papers.errors.length +
									"</strong> not upload!",
								type: "success",
								showCancelButton: false,
								confirmButtonText: "Ok",
...
function delete_bib(value) {
	...
	Swal.fire({
		...
		if (result.value) {
			...
			Swal.fire("File Deleted!", "<strong>File Deleted</strong>", "success");
		}
	});
}
...
function csvJSON(csv) {
	...
	for (let i = 1; i < lines.length; i++) {
		...
		if (current_line.length == 10) {
			...
			errors.push(
				"Article " + i + " in the wrong format, containing some in the title",
			);
		}
	}
